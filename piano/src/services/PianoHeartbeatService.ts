...
} from '../../src/services/heartbeat/HeartbeatService.js';
...
import { TASK_STATUS, DATABASE_TABLES } from '../../src/config/constants.js';
...
export class PianoHeartbeatService /* extends HeartbeatService */ {
  ...
  constructor(db: DatabaseClient, config?: PianoHeartbeatConfig) {
    ...
    const opencodeUrl =
      config?.opencodeUrl || Config.getInstance().getTransportConfig().opencodeApiUrl;
    if (opencodeUrl) {
   ...
   */
  async executePianoTask(taskId: string, title: string, description?: string): Promise<void> {
  ...
  private async executeInternalTask(
    ...
    logger.info(`[PianoHeartbeat] Delegating to internal AI for task: ${title}`);
  }
