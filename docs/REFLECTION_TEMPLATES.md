...
Use these templates with the `areflect` CLI command:

```bash
node dist/cli/index.js areflect "[LEARN] insight: Your insight context: Context"
```
...
4. **Save Immediately**: Run areflect right after completing work
...
```bash
# After fixing a bug
node dist/cli/index.js areflect "[LEARN] insight: Fixed null pointer in UserService - added null check before accessing user.profile context: src/services/UserService.ts:142"

# After implementing a feature
node dist/cli/index.js areflect "[LEARN] insight: Implemented caching using Redis with 5min TTL context: src/services/CacheService.ts [PROMPT_UPDATE] current: No caching suggested: Add Redis caching for frequently accessed data reason: Reduces database load by 80%"

# After finding an issue
node dist/cli/index.js areflect "[ISSUE] title: Missing input validation in API endpoints type: bug severity: high description: Need to add validation for all user inputs"
```
