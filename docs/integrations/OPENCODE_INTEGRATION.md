...
## nupi-action 插件 (OpenCode 提醒)

> 实现 OpenCode 依赖 NUPI 的桥梁：自动向 AI 推送待处理任务和广播

### 概述

nupi-action 插件在 OpenCode 会话中自动注入提醒，让 AI 主动检查待处理任务和广播。
...
# 创建插件文件
cat > ~/.config/opencode/plugins/nupi-action.ts << 'EOF'
...
export const NezhaActionPlugin: Plugin = async (ctx) => {
  ...
  const startIntervalReminder = () => {
    ...
    console.log(`[nupi-action] 固定间隔提醒已启动 (${REMINDER_INTERVAL_MS / 1000 / 60} 分钟)`);
  };

  return {
    event: async ({ event }) => {
      console.log("[nupi-action] Event received:", event.type);

      if (event.type === "session.created") {
        console.log("[nupi-action] Session created, 事件断点提醒...");
        doLearnReminder();
        doTaskCheck();
        doBroadcastReminder();
      ...
      if (event.type === "session.idle") {
        console.log("[nupi-action] Session idle detected!");
        doLearnReminder();
        doTaskCheck();
        doBroadcastReminder();
      }
...
# 应该看到：
# [nupi-action] Plugin loaded successfully
# [nupi-action] Event received: session.created
# [nupi-action] 任务提醒已注入
```
