-- Migration 057: Fix audit_direct_insert trigger - wrong column name
-- Issue: Trigger references 'reported_by' but issues table has 'discovered_by'
...
BEGIN
    ...
    ELSIF TG_TABLE_NAME = 'issues' THEN
        v_author := COALESCE(NEW.discovered_by, 'unknown');  -- FIXED: was reported_by
        v_source := 'unknown';  -- issues table has no source column
...
COMMENT ON FUNCTION audit_direct_insert() IS 'Trigger function that audits direct inserts and sends reminders to use atmReflect or CLI. Fixed in migration 057 to use discovered_by instead of reported_by for issues table.';
